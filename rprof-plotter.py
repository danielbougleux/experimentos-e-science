...
if __name__ == '__main__':
    ...
    if args.memory:
        ...
        generate_resource_graph(data_mem, memus, 'memory usage', 'time(s)', 'Memory Usage(MiB)', start_point=args.start_point, final_point=args.final_point, figname='MemoryUsage', output_path=args.output_dir)

    if args.disk:

        data_disk = structure_data(f'{data_dir}/disk.csv')

        generate_resource_graph(data_disk, diskreadc, 'disk read operations', 'time(s)', 'Disk reads(nops)', start_point=args.start_point, final_point=args.final_point, isRate=True, output_path=args.output_dir, figname='DiskReadOps')

        generate_resource_graph(data_disk, diskreadb, 'disk read in MebiBytes', 'time(s)', 'Disk reads(MiB)', start_point=args.start_point, final_point=args.final_point,isRate=True, output_path=args.output_dir, figname='DiskReadMiB')

        generate_resource_graph(data_disk, diskwritec, 'disk write operations', 'time(s)', 'Disk writes(nops)', start_point=args.start_point, final_point=args.final_point,isRate=True, output_path=args.output_dir, figname='DiskWriteOps')

        generate_resource_graph(data_disk, diskwriteb, 'disk write in MebiBytes', 'time(s)', 'Disk writes(MiB)', start_point=args.start_point, final_point=args.final_point,isRate=True, output_path=args.output_dir, figname='DiskWriteMiB')

    if args.cpu:

        data_cpu = structure_data(f'{data_dir}/cpu.csv')

        generate_resource_graph(data_cpu, iowait, 'iowait', 'time(s)', 'iowait(s)', start_point=args.start_point, final_point=args.final_point,isRate=True, output_path=args.output_dir, figname='iowait')

        generate_resource_graph(data_cpu, cpuusage, 'cpu usage', 'time(s)', 'cpu usage(%)', start_point=args.start_point, final_point=args.final_point,isRate=True, output_path=args.output_dir, figname='cpuusage')
